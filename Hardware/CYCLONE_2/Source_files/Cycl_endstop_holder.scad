...
//	screwWallSep = 2.75;
	...
	holderX = holderX_size;
	holderY = holderY_size;
	holderZ = holderZ_size;
	screwWallSep = endstop_screwWallSep; 
        screwsize = endstop_holder_screwsize;   
	...
	if(holes) {
		...
		translate([0,screwWallSep,3]) {
			translate([3.7,0,0])
				rotate([90,0,0])
					hole_for_screw(size=screwsize,length=15,nutDepth=shortNuts?5:0,nutAddedLen=shortNuts?0:5,captiveLen=10,rot=90);
			translate([22.7,0,0])
				rotate([90,0,0])
					hole_for_screw(size=screwsize,length=15,nutDepth=shortNuts?5:0,nutAddedLen=shortNuts?0:5,captiveLen=10,rot=90);
		}
		...
		translate([0,screwWallSep,3]) {
			translate([3.7,0,0])
				rotate([90,0,0])
					screw_and_nut(size=screwsize,length=10,nutDepth=0, rot=90, echoPart=true);
			translate([22.7,0,0])
				rotate([90,0,0])
					screw_and_nut(size=screwsize,length=10,nutDepth=0, rot=90, echoPart=true);
		}
